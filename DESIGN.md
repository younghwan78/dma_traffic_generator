...
## 7.2 OTF gate & Bottleneck Synchronization

OTF 링크는 producer instance 완료 시점 기준으로 consumer instance의 시작 가능 시점을 정합니다.

추가로, OTF로 연결된 전체 IP들은 하나의 그룹(Connected Component)으로 묶이며, 그룹 내에서 자체 처리 속도가 가장 느린 IP(Bottleneck)의 처리 시간(`natural_duration_ns`)을 기준으로 그룹 내 모든 IP의 데이터 출력 간격(`line_step_ns`)이 강제로 지연(Stretch)되어 동기화됩니다. 이를 통해 실제 하드웨어의 파이프라인 Backpressure 현상을 정확하게 모델링합니다.
...
- source write DMA 완료 시점에 추가적인 SW Task 지연 시간(`delta_ns`)을 더한 시점 이후에 시작
...
- 기본적으로 전체 시스템 관점에서 모든 DMA 트랜잭션을 시간 순으로 통합 출력
- `run` 명령의 `--split-by-dma` 옵션 사용 시 각 DMA 단위로 쪼개진 개별 파일들 동시 생성
...
- 그래프의 X축(시간)은 시나리오의 전체 수행 시간(`duration_ms`)으로 고정되어, 트래픽이 일찍 종료되어도 전체 프레임 관점의 비율을 유지합니다.
